...
using namespace cv;
using namespace std;
Mat dst, gray_src, inv_dis, src_image, gray_img, brul;
char input_image[] = "input image";
char output_image[] = "output image";
int width;
int height;

void swapImage(const Mat &binary);
...
void onMouse(int event, int x, int y, int flags, void *utsc) {
    ...
    {
        cout << "响应鼠标左键抬起事件" << endl;
        photo = false;
        Rect rect(65, 65, imageWidth - 130, imageHeight - 130);
        Mat image_roi = image(rect);
        imshow("rect", image_roi);
        buildImage(image_roi);
    }
}
void show2() {
    ...
    while (photo) {
        ...
        if (!image.empty()) {
            ...
            polylines_v.push_back(cv::Point(60, 60));
            polylines_v.push_back(cv::Point(60, imageHeight - 60));
            polylines_v.push_back(cv::Point(imageWidth - 60, imageHeight - 60));
            polylines_v.push_back(cv::Point(imageWidth - 60, 60));
...
//            cout << "ln  : " << ln << endl;
    ...
    line(src_image, Point(topLine[0], topLine[1]), Point(topLine[2], topLine[3]), Scalar(0, 0, 255),
         2,
         LINE_AA,
         0);//white
    line(src_image, Point(rightLine[0], rightLine[1]), Point(rightLine[2], rightLine[3]), Scalar(0, 255, 0),
         2,
         LINE_AA, 0);//white
    line(src_image, Point(bottomLine[0], bottomLine[1]), Point(bottomLine[2], bottomLine[3]), Scalar(255, 0, 0),
         2,
         LINE_AA, 0);//white
    line(src_image, Point(leftLine[0], leftLine[1]), Point(leftLine[2], leftLine[3]), Scalar(255, 255, 255),
         2,
         LINE_AA, 0);//white
    // 拟合四条直线方程
    float k1, c1;
